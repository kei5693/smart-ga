...
(function() {
	var Class = {
		...
		/* jsacc */	initAccordionOnce: function initAccordionOnce(){
			$(document)
				.off('showOneAcc', '.jsacc > .view')
				.on('showOneAcc', '.jsacc > .view', function(e) {
					$(this).slideDown(200).attr('aria-hidden', "false");
					e.preventDefault();
					e.stopPropagation();
				})
				.off('hideOneAcc', '.jsacc > .view')
				.on('hideOneAcc', '.jsacc > .view', function(e) {
					$(this).slideUp(200).attr('aria-hidden', "true");
					e.preventDefault();
					e.stopPropagation();
				})
				.off('click', '.jsacc .header')
				.on('click', '.jsacc .header', function(e) {
					var $obj = $(this);
		...
		/* 레이아웃 세팅 */	setLayout: function setLayout() {
			...
			if($('.swiper-container').length>0){
				...
				$swiperContainer.each(function(){
					...
					if($swiperContainer.hasClass('carinfo-swiper')){
						
						if($(this).find('.swiper-slide').length < 2) $(this).addClass('one-slide');
